...
class ShikraLlamaModel(LlamaModel):
    ...
    def forward(
        ...
        if vision_tower is not None and (input_ids.shape[1] != 1 or self.training) and images is not None:
            ...
            dev = images.device
            dt = images.dtype  # 你传的是 .half()，这里通常是 torch.float16

            # 取出真正的视觉塔模块
            vt = vision_tower[0] if isinstance(vision_tower, (list, tuple)) else vision_tower

            # 若视觉塔不在同一 device/dtype，则移动
            if next(vt.parameters()).device != dev or next(vt.parameters()).dtype != dt:
                vt.to(device=dev, dtype=dt, non_blocking=True)

            # 如果还有 images_cd，也保证一致
            # with torch.no_grad():
        ...
        return super(ShikraLlamaModel, self).forward(
            ...
            vad=vad, key_position=key_position,
...
class ShikraLlamaForCausalLM(LlamaForCausalLM):
    ...
    def prepare_inputs_for_generation_icd(
            self, input_ids, past_key_values=None, attention_mask_icd=None, inputs_embeds=None, **kwargs
    ):
        if past_key_values:
            input_ids = input_ids[:, -1:]
        ...
        if inputs_embeds is not None and past_key_values is None:
            ...
            model_inputs = {"input_ids": kwargs.get("input_ids_cd", None)}
...
class ShikraBase(BaseModel):
    ...
    def quantization_check(
        ...
        if llama_model.model.vision_tower[0].device != torch.device('meta'):
            llama_model.model.vision_tower[0].to(dtype=dtype, device="cpu")
        else:
