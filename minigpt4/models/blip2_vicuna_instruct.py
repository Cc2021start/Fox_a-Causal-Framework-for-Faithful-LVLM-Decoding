...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def generate(
        ...
        if isinstance(prompt, str):
            # prompt = prompt.split("<ImageHere>")[-1]
            # prompt = [prompt] * bs
            prompt = prompt.replace('<ImageHere>', '').replace('<Img>', '').replace('</Img>', '')
            prompt = [prompt] * bs
        else:
            # prompt = [p.split("<ImageHere>")[-1] for p in prompt]
            prompt = [p.replace('<ImageHere>', '').replace('<Img>', '').replace('</Img>', '') for p in prompt]
            assert len(prompt) == bs, "The number of prompts must be equal to the batch size."
        
        if prompt_cd is not None:
            if isinstance(prompt_cd, str):
                prompt_cd = prompt_cd.split("<ImageHere>")[-1]
                prompt_cd = [prompt_cd] * bs
            else:
                prompt_cd = [p.split("<ImageHere>")[-1] for p in prompt_cd]
                assert len(prompt_cd) == bs, "The number of prompts must be equal to the batch size."
        ...
        if self.qformer_text_input:
            ...
            if prompt_cd is not None:
                ...
                query_atts = torch.ones(query_tokens.size()[:-1], dtype=torch.long).to(image.device)
                Qformer_atts_cd = torch.cat([query_atts, text_Qformer_cd.attention_mask], dim=1)
        ...
        if image.dim() == 5:
            ...
            with self.maybe_autocast():
                image_embeds = self.ln_vision(self.visual_encoder(image))#shapetorch.Size([1, 257, 1408])
            image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device)
            ...
            atts_llm = torch.ones(inputs_llm.size()[:-1], dtype=torch.long).to(image.device)
            if prompt_cd is not None:
                inputs_llm_icd = self.llm_proj(query_output_cd.last_hidden_state[:,:query_tokens.size(1),:])#.index_select(dim=1, index=torch.tensor([4,31]).to(image.device))
                # inputs_llm = torch.cat([inputs_llm[:,-i,:].unsqueeze(1) for i in range(inputs_llm.shape[1])], dim=1)
                atts_llm_icd = torch.ones(inputs_llm_icd.size()[:-1], dtype=torch.long).to(image.device)
        ...
        if prompt_cd is not None:
            llm_tokens_icd = self.llm_tokenizer(
                prompt_cd,
                padding="longest",
                return_tensors="pt"
            ).to(image.device)
        ...
        with self.maybe_autocast():
            ...
            ## cd_comments: format
            if prompt_cd is not None:
                inputs_embeds_icd = self.llm_model.get_input_embeddings()(llm_tokens_icd.input_ids)
                inputs_embeds_icd = torch.cat([inputs_llm_icd, inputs_embeds_icd], dim=1)
                # attention_mask_icd = torch.cat([atts_llm_icd, llm_tokens_icd.attention_mask], dim=1)
            if images_cd is not None:
            ...
            if key_position is None:
                key_position = {
                    "image_start": 1, 
                    "image_end": inputs_llm.shape[1], 
                    "response_start": inputs_embeds.shape[1]
                }

            outputs = self.llm_model.generate(
                inputs_embeds=inputs_embeds,
                inputs_embeds_cd=inputs_embeds_icd if prompt_cd is not None else None,
                attention_mask=attention_mask,
                ...
                # input_ids_cd=inputs_embeds_icd if prompt_cd is not None else None,
